...
void _insert(Node *present, int k)
{
    ...
    if (present->leaf)
    {
        ...
        while (1)
        {
            if (leaf_index < i && k > present->keys[leaf_index])
            {
                leaf_index++;
            }
            else
        ...
        for (int j = i - 1; j >= leaf_index; j--)
        {
            present->keys[j + 1] = present->keys[j];
        }
        present->keys[leaf_index] = k;
        present->n++;
    ...
    else // If this node is not leaf
    {
        ...
        while (1)
        {
            if (index < i && k > present->keys[index])
            {
                index++;
            }
            else
...
Node *_splitChild(Node *present)
{
    ...
    splitIdx = (int)floor((double)t / 2);

    right->n = (int)ceil((double)t / 2) - offset;
    ...
    if (present->parent != NULL)
    {
        ...
        for (parentIdx = 0; parentIdx <= currentParent->n && currentParent->child[parentIdx] != present; parentIdx++)
            ;

        for (i = currentParent->n; i > parentIdx; i--)
        {
            currentParent->child[i + 1] = currentParent->child[i];
    ...
    {
        right->child[i - splitIdx - offset] = present->child[i];
    ...
    for (i = splitIdx + offset; i < t; i++)
    {
        right->keys[i - splitIdx - offset] = present->keys[i];
    }
...
void _remove(Node *present, int k)
{
    ...
    else
    {
        ...
        if (i == present->n)
        {
            if (!present->leaf)
            {
                _remove(present->child[present->n], k);
            }
        }

        else if (!present->leaf && present->keys[i] == k)
        {
        ...
        else if (present->leaf && present->keys[i] == k)
        {
            ...
            if (i == 0 && present->parent != NULL)
            {
                ...
                if (present->n == 0)
                {
                    ...
                    {
                        smallest = parent->child[parentIdx + 1]->keys[0];
                    }
                    else{
                        smallest=-1;
                    }
                ...
                while(parent!=NULL){
                    ...
                    Node* grandparnet=parent->parent;
                    for(parentIdx=0;grandparnet!=NULL&&grandparnet->child[parentIdx]!=parent;parentIdx++);
